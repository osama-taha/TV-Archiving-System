        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if ((!blockPaint) && ((bars != null) && (bars.GetLength(0) > 0) && (barCount > 0)))
            {
                ...
                for (int i = 0; i < bars.Length; i++)
                {
                    ...
                    #region "INNER BARS"
                    //for (int j = 0; j < bars[i].GetLength(0); j++)
                    //{
                        float xStart = 0;
                        float zwidth = 1;
                        ...
                        /*
                        //We only need to transform in the X-direction
                         * //This is probably cleaner code?
                        PointF p1 = new PointF(xStart, 0);
                        SizeF s1 = new SizeF(zwidth, 0);
                        PointF p2 = Transform(m_data_rect, m_screen_rect, p1);
                        SizeF s2 = Transform(m_data_rect, m_screen_rect, s1);
                        barSegRect = new RectangleF(p1.X, newY, s1.Width, zBarHeight / 3);
                        */

                        //if (bars[i].goodData)
                        //{
                        //    //if (RecTypeSchedule && bars[i].recordingType == 0) //0
                        //    //{
                        //    //    //barSegRect = new RectangleF(barSegRect.X, barSegRect.Y + 2 * barSegRect.Height, barSegRect.Width, barSegRect.Height);
                        //    //    barSegRect = new RectangleF(barSegRect.X, barSegRect.Y + (1 * barOffsetY), barSegRect.Width, barSegRect.Height);
                        //    //    using (LinearGradientBrush lgbBarLive = new LinearGradientBrush(barSegRect,
                        //    //        barLiveOuterColor, barLiveInnerColor, LinearGradientMode.Horizontal))
                        //    //    {
                        //    //        lgbBarLive.WrapMode = WrapMode.TileFlipXY;
                        //    //        e.Graphics.FillRectangle(lgbBarLive, barSegRect);
                        //    //    }
                        //    //}
                        //    //if (RecTypeLive && bars[i][j].recordingType == 2)
                        //    //{
                        //    //    barSegRect = new RectangleF(barSegRect.X, barSegRect.Y + (5 * barOffsetY) + (2 * (barSegRect.Height)), barSegRect.Width, barSegRect.Height);
                        //    //    using (LinearGradientBrush lgbBarSchedule = new LinearGradientBrush(barSegRect,
                        //    //        barScheduledOuterColor, barScheduledInnerColor, LinearGradientMode.Horizontal))
                        //    //    {
                        ...
                        if (bars[i].goodData)
                        {
                                ...
                                barMediaOuterColor, barMediaInnerColor, LinearGradientMode.Horizontal))
                            {
                                ...
                                e.Graphics.FillRectangle(lgbBarMedia, barSegRect);
                            }
                        }
